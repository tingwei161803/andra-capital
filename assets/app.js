...
(function () {
  ...
  function boot() {
    ...
    function proseParas(arr) {
      return (arr || []).map(function (para) {
        return '<p class="prose-para">' + esc(t(para)) + "</p>";
      }).join("");
    }
    function factRows(rows) {
      return '<div class="factlist">' + (rows || []).map(function (row) {
        return '<div class="fact" data-item>' +
          '<div class="fact__head">' +
            (row.icon ? '<span class="material-symbols-rounded fact__icon" aria-hidden="true">' + esc(row.icon) + "</span>" : "") +
            '<span class="fact__label">' + esc(t(row.label)) + "</span>" +
          "</div>" +
          '<div class="fact__main">' +
            (t(row.value) ? '<span class="fact__value">' + esc(t(row.value)) + "</span>" : "") +
            (t(row.note) ? '<span class="fact__note">' + esc(t(row.note)) + "</span>" : "") +
          "</div>" +
          '<div class="fact__status">' + badge(row.status) + "</div>" +
        "</div>";
      }).join("") + "</div>";
    }
    function refsBlock(refs) {
      if (!refs || !refs.length) return "";
      var lab = L.state.lang === "en" ? "Sources" : "來源";
      return '<aside class="refs"><h2 class="refs__title">' + lab + "</h2><ul>" +
        refs.map(function (ref) {
          return '<li><a class="row-link" href="' + esc(ref.url) + '" target="_blank" rel="noopener">' +
            esc(t(ref.label)) + "</a></li>";
        }).join("") + "</ul></aside>";
    }
    ...
    var RENDERERS = {
      ...
      hub: function (p) {
        var stats = (p.stats || []).map(function (s) {
          return '<div class="hero__stat" data-item>' +
            '<b class="hero__stat-value" data-count="' + esc(String(s.value)) + '"' +
              ' data-prefix="' + esc(s.prefix || "") + '" data-suffix="' + esc(s.suffix || "") + '"' +
              ' data-decimals="' + esc(String(s.decimals || 0)) + '">0</b>' +
      ...
      article: function (p) {
        ...
        var body = (p.sections || []).map(function (s) {
          var blocks = (s.blocks || []).map(function (b) {
            ...
            if (b.type === "facts") return factRows(b.rows);
            if (b.type === "ul") {
      ...
      /* ---- timeline: dated event cards down a rail (+ optional prose intro/outro) ---- */
      timeline: function (p) {
        ...
        return head(p) +
          (p.intro ? '<div class="prose page-intro">' + proseParas(p.intro) + "</div>" : "") +
          '<ol class="timeline">' + items + "</ol>" +
          (p.outro ? '<div class="prose page-outro">' + proseParas(p.outro) + "</div>" : "") +
          refsBlock(p.refs);
      },

      /* ---- facts: editorial fact-check sections with status badges ---- */
      facts: function (p) {
        var sections = (p.sections || []).map(function (s) {
          return '<section class="factsec">' +
            '<h2 class="factsec__heading">' + esc(t(s.heading)) + "</h2>" +
            factRows(s.rows) + "</section>";
        }).join("");
        return head(p) +
          (p.intro ? '<div class="prose page-intro">' + proseParas(p.intro) + "</div>" : "") +
          sections + refsBlock(p.refs);
      },

      /* ---- people: numbered editorial rows grouped by era; dialog per person ---- */
      people: function (p) {
        var n = 0;
        var groups = (p.groups || []).map(function (g) {
          var rows = (g.members || []).map(function (m) {
            n += 1;
            return '<article class="person card" tabindex="0" role="button" data-item data-slug="' + esc(m.slug) + '" ' +
                'aria-label="' + esc(t(m.name)) + '">' +
              '<span class="person__num" aria-hidden="true">' + (n < 10 ? "0" + n : n) + "</span>" +
              '<div class="person__main">' +
                '<h3 class="person__name">' + esc(t(m.name)) + "</h3>" +
                '<p class="person__role">' + esc(t(m.role)) + "</p>" +
              "</div>" +
              '<div class="person__side">' + badge(m.status) +
                '<span class="person__cta">' + (L.state.lang === "en" ? "Fact-check →" : "查證 →") + "</span>" +
              "</div>" +
            "</article>";
          }).join("");
          return '<section class="peoplegroup">' +
            '<h2 class="factsec__heading">' + esc(t(g.heading)) + "</h2>" +
            '<div class="peoplelist">' + rows + "</div></section>";
        }).join("");
        return head(p) + groups;
      },

      /* ---- portfolio: evidence-tier groups of company rows ---- */
      portfolio: function (p) {
        var tiers = (p.tiers || []).map(function (tier) {
          var rows = (tier.companies || []).map(function (c) {
            return '<div class="holding" data-item>' +
              '<span class="holding__name">' + esc(c.name) + "</span>" +
              '<span class="holding__sector">' + esc(t(c.sector)) + "</span>" +
              '<span class="holding__note">' + esc(t(c.note)) + "</span>" +
            "</div>";
          }).join("");
          return '<section class="tiergroup tiergroup--' + esc(tier.status) + '">' +
            '<div class="tiergroup__head">' +
              '<h2 class="tiergroup__label">' + esc(t(tier.label)) + "</h2>" + badge(tier.status) +
            "</div>" +
            '<p class="tiergroup__desc">' + esc(t(tier.desc)) + "</p>" +
            '<div class="holdings">' + rows + "</div></section>";
        }).join("");
        return head(p) + tiers +
          (p.outro ? '<div class="prose page-outro">' + proseParas(p.outro) + "</div>" : "") +
          refsBlock(p.refs);
      ...
      sources: function (p) {
        ...
        var groups = (p.groups || []).map(function (g) {
          var links = (g.links || []).map(function (ln) {
            return '<li data-item><a class="row-link" href="' + esc(ln.url) + '" target="_blank" rel="noopener">' +
              esc(t(ln.label)) + "</a></li>";
          }).join("");
          return '<section class="srcgroup">' +
            '<h2 class="factsec__heading">' + esc(t(g.heading)) + "</h2>" +
            '<ul class="srclist">' + links + "</ul></section>";
        }).join("");
        return head(p) +
          (legend ? '<div class="legend">' + legend + "</div>" : "") + groups;
      },
    ...
    var WIRE = {
      ...
      people: function (p) {
        var members = [];
        (p.groups || []).forEach(function (g) { members = members.concat(g.members || []); });
        function findMember(slug) {
          return members.filter(function (m) { return m.slug === slug; })[0] || null;
        }
        function openMember(slug) {
          var m = findMember(slug); if (!m) return;
          var dlg = L.dialog(), body = document.getElementById("dialogBody");
          var facts = (m.facts || []).map(function (f) {
            return '<div class="fact">' +
              '<div class="fact__main">' +
                '<span class="fact__value">' + esc(t(f.claim)) + "</span>" +
                (t(f.note) ? '<span class="fact__note">' + esc(t(f.note)) + "</span>" : "") +
              "</div>" +
              '<div class="fact__status">' + badge(f.status) + "</div></div>";
          }).join("");
          var refs = (m.refs || []).map(function (ref) {
            return '<li><a class="row-link" href="' + esc(ref.url) + '" target="_blank" rel="noopener">' +
              esc(t(ref.label)) + "</a></li>";
          }).join("");
          body.innerHTML = '<h2 id="dialogTitle" class="person__name">' + esc(t(m.name)) + "</h2>" +
            '<p class="person__role">' + esc(t(m.role)) + "</p>" +
            '<p class="dialog__summary">' + esc(t(m.summary)) + "</p>" +
            (facts ? '<div class="factlist factlist--dialog">' + facts + "</div>" : "") +
            (refs ? '<h3 class="refs__title">' + (L.state.lang === "en" ? "Sources" : "來源") + "</h3><ul class=\"srclist\">" + refs + "</ul>" : "");
          if (!dlg.open) dlg.showModal();
          if (location.hash.slice(1) !== slug) history.replaceState(null, "", "#" + slug);
        }
        [].forEach.call(pageEl.querySelectorAll(".person[data-slug]"), function (row) {
          var slug = row.dataset.slug;
          row.addEventListener("click", function () { openMember(slug); });
          row.addEventListener("keydown", function (e) {
            if (e.key === "Enter" || e.key === " ") { e.preventDefault(); openMember(slug); }
          });
        });
        var dlg = L.dialog();
        function onClose() {
          var slug = location.hash.slice(1);
          if (slug && findMember(slug)) history.replaceState(null, "", location.pathname + location.search);
        }
        dlg.addEventListener("close", onClose);
        function syncHash() {
          var slug = location.hash.slice(1);
          if (slug && findMember(slug)) openMember(slug);
        }
        var onHash = function () { syncHash(); };
        window.addEventListener("hashchange", onHash);
        teardowns.push(function () {
    ...
    function animateCounters() {
      ...
      function run(el) {
        ...
        function fmt(n) { return pre + n.toFixed(dec) + suf; }
        function step(ts) {
